...
visualization_msgs::Marker MapManager::get_visOstacle() {
    ...
    obs_list.color.a = 0.4;// 透明度

    obs_list.color.r = 0.0;
    obs_list.color.g = 0.0;
...
bool MapManager::isOccupied(const int &idx_x, const int &idx_y, const int &idx_z) const
{
    return (idx_x >= 0 && idx_x < GLX_SIZE && idx_y >= 0 && idx_y < GLY_SIZE && idx_z >= 0 && idx_z < GLZ_SIZE &&
            (data[idx_x + idx_y * GLX_SIZE + idx_z*GLXY_SIZE] == 1));
}

bool MapManager::isFree(const int &idx_x, const int &idx_y, const int &idx_z) const
{
    return (idx_x >= 0 && idx_x < GLX_SIZE && idx_y >= 0 && idx_y < GLY_SIZE && idx_z >= 0 && idx_z < GLZ_SIZE &&
            (data[idx_x + idx_y * GLX_SIZE + idx_z*GLXY_SIZE] < 1));
}