...
void BezierTrajGenerator::timeAllocation()
{
    ...
    for(int i = 1; i < (int)_corridor.size(); i++)
        points.push_back(_corridor[i].center);

    points.push_back (_end_pt);

    double _Vel = _Vel_max * 0.6;
    double _Acc = _Acc_max * 0.6;
...
Eigen::MatrixXd BezierTrajGenerator::getAeq()
{
     ...
     * 贝赛尔曲线求导(系数形式为杨辉三角)
     */
    Aeq_start<<_corridor[0].t,    0,  0, 0, 0, 0, 0, 0,  //  p
            -n_order,   n_order,  0, 0, 0, 0, 0, 0,   //  v
            n_2/_corridor[0].t, -2*n_2/_corridor[0].t, n_2/_corridor[0].t, 0, 0, 0, 0, 0;   //  a

    MatrixXd Aeq_end(3, n_coeff);
    Aeq_end<<0,    0,  0, 0, 0, 0, 0, _corridor.back().t,  //  p
            0,   0,  0, 0, 0, 0, -1, 1,   //  v
            0, 0, 0, 0, 0, 1/_corridor.back().t, -2*1/_corridor.back().t, 1/_corridor.back().t;   //  a

//    Aeq_end<<0,    0,  0, 0, 0, 0, 0, _corridor.back().t,  //  p
//            0,   0,  0, 0, 0, 0, -n_order, n_order,   //  v
//            0, 0, 0, 0, 0, n_2/_corridor.back().t, -2*n_2/_corridor.back().t, n_2/_corridor.back().t;   //  a
    ...
    for(int i=0;i<n_seg-1;++i)
    {
        ...
        /*
         * TODO: 位置可能也得重新加约束,跟_corridor[i].t有关
         * */
...
//        Aeq_con_v(i,index-1) = -n_order;    Aeq_con_v(i,index) = 2*n_order; Aeq_con_v(i,index+1) = -n_order;
        Aeq_con_v(i,index-1) = -1;    Aeq_con_v(i,index) = 2*1; Aeq_con_v(i,index+1) = -1;

        //  a
//        Aeq_con_a(i,index-2) = n_2;    Aeq_con_a(i,index-1) = -2*n_2; Aeq_con_a(i,index+1) = 2*n_2;  Aeq_con_a(i,index+2) = -n_2;
        Aeq_con_a(i,index-2) = 1/_corridor[i].t;    Aeq_con_a(i,index-1) = -2/_corridor[i].t;
        Aeq_con_a(i,index) = 1/_corridor[i].t-1/_corridor[i+1].t;
        Aeq_con_a(i,index+1) = 2/_corridor[i+1].t;  Aeq_con_a(i,index+2) = -1/_corridor[i+1].t;
    }
...
Eigen::MatrixXd BezierTrajGenerator::getAieq() {
    ...
    double d1 = n_order;//(double)1/_max_t_seg;//n_order/_max_t_seg;
    double d2 = n_order*(n_order-1)/_max_t_seg;//(double)1/(_max_t_seg*_max_t_seg);//n_order*(n_order-1)/(_max_t_seg*_max_t_seg);
...
Eigen::VectorXd BezierTrajGenerator::getbieq_plus(int axis){
    ...
    for(int i = 0;i<n_seg;++i)
    {
        ...
        {
            bieq_p_plus(first_coeff_index+k) = _corridor[i].max_pt(axis)/scale_k;
        }
        /**
        ...
        if(i!=n_seg-1)
        {
            bieq_p_plus(first_coeff_index+n_coeff-2) = min(_corridor[i].max_pt(axis),_corridor[i+1].max_pt(axis))/scale_k;
        }
    }
...
Eigen::VectorXd BezierTrajGenerator::getbieq_minus(int axis){
    ...
    for(int i = 0;i<n_seg;++i)
    {
        ...
        for(int k = 0;k<n_coeff-2;++k)
        {
            bieq_p_minus(first_coeff_index+k) = _corridor[i].min_pt(axis)/scale_k;
        }
        /**
         * 对每段第 n_coeff 个点进行限制
         * 第n_coeff点是轨迹间共同控制点
        ...
        if(i!=n_seg-1)
        {
            bieq_p_minus(first_coeff_index+n_coeff-2) = max(_corridor[i].min_pt(axis),_corridor[i+1].min_pt(axis))/scale_k;
        }
    }
